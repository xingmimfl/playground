...
int main(){
  ...
  cv::Mat a_crop_image_copy = a_crop_image.clone();
  ...
  for(int i=0; i<landmarks.size(); i++){
      ...
      float y = a_point.y;
      //caution!!!! must use double type here, float is not right!!!!
      landmarks[i].x = x * M_inverse.at<double>(0,0) + y * M_inverse.at<double>(0,1);
      landmarks[i].y = x * M_inverse.at<double>(1,0) + y * M_inverse.at<double>(1,1);
  }
  for(int i=0; i<landmarks.size(); i++){
    landmarks[i].x += crop_w/2;
    landmarks[i].y += crop_h/2;
  ...
  for(int i = 0; i<landmarks.size(); i++){
    cv::Point2f a_point = landmarks[i];
    cv::circle(a_crop_image_copy, cv::Point(int(a_point.x), int(a_point.y)), 1, cv::Scalar(0, 255, 0), -1);
  }
  
  cv::imwrite("hh.png", a_crop_image_copy);
}
